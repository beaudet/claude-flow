...
export class ClaudeCodeMCPWrapper {
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler('tools/call', async (request) => this.handleToolCall(request.params.name, request.params.arguments || {}));
    }
    async getTools() {
        const tools = [];
        ...
        if (toolName === 'Task') {
            ...
            if (modeMatch) {
                ...
                if (mode) {
                    ...
                    try {
                        const result = await global.executeSparcMode(modeName, args.prompt || '', mode.tools || [], {});
                        return {
                            content: [
                                {
                                    type: 'text',
